...
if [ "$KOKORO_GITHUB_PULL_REQUEST_NUMBER" == "" ]; then
  go test -timeout=30m -parallel=4 -tags=integration -run TestAgentIntegration -commit="$COMMIT" -branch="$BRANCH" -repo="$REPO"
else 
  go test -timeout=30m -parallel=4 -tags=integration -run TestAgentIntegration -commit="$COMMIT" -pr="$KOKORO_GITHUB_PULL_REQUEST_NUMBER"
fi
